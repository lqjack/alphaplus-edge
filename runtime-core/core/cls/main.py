...
def stock_data(stock):
    ...
    res_json = s.get(s_url + '&sign=' + encrypts(s_url),headers=headers,timeout=10).json()
    logger.info('--------------------',time.strftime('%Y.%m.%d-%H:%M:%S'),res_json['data']['secu_name'],'获取成功','--------------------')
    return res_json['data']
def run(s_url):
    plate_list=plate_parse(s_url)
    for plate in plate_list:
        try:
            logger.info('---------------', time.strftime('%Y.%m.%d-%H:%M:%S'),'开始获取板块:',plate['bk_name'],'---------------')
            stock_list=stock_parse(plate)
        except Exception as e:
            logger.error('***error:{},股票名及链接请求失败:{}'.format(e,plate['bk_name']))
            continue
        for stock in stock_list:
            try:
                result=stock_data(stock)
                logger.info(result)   #####最终结果
            except Exception as e:
                logger.info('***error:{},股票数据请求失败:{}'.format(e,stock['stock_name']))
